...
from .expansions import (stack_2D_to_3D_expansion,
                         rotate_2D_to_3D_expansion,
                         cone_2D_to_3D_expansion,
                         helix_2D_to_3D_expansion)
from .trajectory2D import (initialize_2D_radial,
...
def initialize_3D_from_2D_expansion(basis, expansion, Nc, Ns, nb_repetitions,
                                    basis_kwargs={}, expansion_kwargs={}):
...
def initialize_3D_cones(Nc, Ns, tilt="golden", in_out=False, nb_zigzags=5, nb_overlaps=0):
    # Initialize first cone characteristics
    radius = np.linspace(-KMAX if (in_out) else 0, KMAX, Ns)
    angles = np.linspace(-2 * np.pi * nb_zigzags if (in_out) else 0,
                          2 * np.pi * nb_zigzags, Ns)
    trajectory = np.zeros((Nc, Ns, 3))
    trajectory[:, :, 0] = radius
    trajectory[:, :, 1] = radius * np.cos(angles) \
                          * (np.abs(nb_overlaps) + 1) \
                          * 2 * np.pi / Nc ** (2 / 3) / (1 + in_out)
    trajectory[:, :, 2] = radius * np.sin(angles) \
                          * (np.abs(nb_overlaps) + 1) \
                          * 2 * np.pi / Nc ** (2 / 3) / (1 + in_out)
