...
def _subspace_gram_eig(
    ...
    if max_eigval == 0:
        raise ValueError(
            "Calibration region has a zero-norm C^H C matrix (empty or "
            "all-zero calibration k-space): cannot estimate coil "
            "sensitivities. Check the trajectory/k-space data and "
            "`calib_width`."
        )
    eigval /= max_eigval
...
def coil_compression(
    ...
    # `eigh` returns eigenvectors as columns of `v` (`v[:, i]` <-> `w[i]`), so
    # sorting/truncating must index columns, not rows.
    si = xp.argsort(w)[::-1]
    w_sorted = w[si]
    v_sorted = v[:, si]
