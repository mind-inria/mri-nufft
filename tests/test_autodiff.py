...
def test_adjoint_and_grad(operator, interface):
    ...
    operator.samples = to_interface(
        operator.samples, interface=interface
    )  # [2048, 2] more samples!!!!
    ksp_data = to_interface(
        kspace_from_op(operator), interface=interface
    )  # y [1, 2048]
    img_data = to_interface(image_from_op(operator), interface=interface)  # [1, 32, 32]
    ksp_data.requires_grad = True
    operator.samples.requires_grad = True

    with torch.autograd.set_detect_anomaly(True):
        adj_data = operator.adj_op(ksp_data).reshape(img_data.shape)  # [ 1, 16, 16]
    ...
    assert torch.allclose(
        gradient_ndft_ktraj, gradient_nufft_ktraj, atol=5e-2
    )  # FIXME: plot the gradient
