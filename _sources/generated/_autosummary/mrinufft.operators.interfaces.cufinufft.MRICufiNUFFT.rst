MRICufiNUFFT
============

.. currentmodule:: mrinufft.operators.interfaces.cufinufft

.. autoclass:: MRICufiNUFFT
   :members:
   :private-members:
   :show-inheritance:
   :special-members: __call__, __add__, __mul__, __matmul__

   
   
   .. rubric:: Methods

   .. autosummary::
      :nosignatures:
   
      ~MRICufiNUFFT.__init__
      ~MRICufiNUFFT.adj_op
      ~MRICufiNUFFT.cg
      ~MRICufiNUFFT.check_shape
      ~MRICufiNUFFT.compute_density
      ~MRICufiNUFFT.compute_smaps
      ~MRICufiNUFFT.data_consistency
      ~MRICufiNUFFT.get_lipschitz_cst
      ~MRICufiNUFFT.make_autograd
      ~MRICufiNUFFT.op
      ~MRICufiNUFFT.pipe
      ~MRICufiNUFFT.toggle_grad_traj
      ~MRICufiNUFFT.with_autograd
      ~MRICufiNUFFT.with_off_resonance_correction
   
   

   
   
   .. rubric:: Attributes

   .. autosummary::
   
      ~MRICufiNUFFT.autograd_available
      ~MRICufiNUFFT.available
      ~MRICufiNUFFT.backend
      ~MRICufiNUFFT.bsize_img
      ~MRICufiNUFFT.bsize_ksp
      ~MRICufiNUFFT.cpx_dtype
      ~MRICufiNUFFT.density
      ~MRICufiNUFFT.dtype
      ~MRICufiNUFFT.eps
      ~MRICufiNUFFT.img_size
      ~MRICufiNUFFT.interfaces
      ~MRICufiNUFFT.ksp_size
      ~MRICufiNUFFT.n_batchs
      ~MRICufiNUFFT.n_coils
      ~MRICufiNUFFT.n_samples
      ~MRICufiNUFFT.ndim
      ~MRICufiNUFFT.norm_factor
      ~MRICufiNUFFT.samples
      ~MRICufiNUFFT.shape
      ~MRICufiNUFFT.smaps
      ~MRICufiNUFFT.uses_density
      ~MRICufiNUFFT.uses_sense
   
   

   .. _sphx_glr_backref_mrinufft.operators.interfaces.cufinufft.MRICufiNUFFT:

   .. minigallery:: mrinufft.operators.interfaces.cufinufft.MRICufiNUFFT
      :add-heading: